...
- **Bekijk de pagina echt in de browser na een UI-wijziging.** `preview_start` met de URL, daarna `navigate` — een kale `navigate` zonder open paneel geeft een melding die klinkt als een beleidsblokkade, en op grond daarvan is meermaals onterecht gemeld dat `stockscreen-janco.fly.dev` geblokkeerd zou zijn. Dat is niet zo. Werkt `screenshot` niet ("not compositing frames"), gebruik dan `get_page_text` of `javascript_tool`. Tests en pyflakes vangen geen presentatiefouten: "kwaliteit +0" waar niets veranderd was en een tabel die niet optelde kwamen alleen door kijken boven water.
